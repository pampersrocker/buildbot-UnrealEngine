...
class Build(BaseUnrealCommand):
  ...
  def start(self):
    command = [ self.getEngineBatchFilesPath(self.build_type), self.target, self.target_platform, self.target_config, self.project_path ]
    if self.wait_mutex:
  ...
  def doSanityChecks(self):
    if isinstance(self.build_type, str) and self.build_type not in self.supported_build_types:
      config.error("build_type '{0}' is not supported".format(self.build_type))
    if isinstance(self.target_config, str) and self.target_config not in self.supported_target_config:
      config.error("target_config '{0}' is not supported".format(self.target_config))
    if isinstance(self.target_platform, str) and self.target_platform not in self.supported_target_platforms:
